...
output: pdf_document
---
github link: https://github.com/eivra-sm/APM1111/blob/main/FA10.md
...
Since all p > 0.05, we fail to reject the null hypothesis of normality. This conclusion is further corroborated by an inspection of the skewness and kurtosis values, which fell within the acceptable range (typically between -1.96 and +1.96). Therefore, Assumption 5 is satisfied.
...
# Convert the summaries to long format for presentation
descriptives_long <- descriptives %>%
  pivot_longer(cols = Valid:`Std. Error of Kurtosis`, names_to = "Statistic", values_to = "Value") %>%
  unite("Time_Margarine", Time, Margarine, sep = "_") %>%
  pivot_wider(names_from = "Time_Margarine", values_from = "Value")
...
kable(
  ...
  add_header_above(c(" " = 1, "Time Points" = 6)) %>%
...
normality_results <- df_long %>%
  group_by(Time, Margarine) %>%
  summarise(
    shapiro_test = list(shapiro_test(Cholesterol)),
    .groups = "drop"
  ) %>%
  mutate(
    Shapiro_Wilk_statistic = map_dbl(shapiro_test, ~ .x$statistic),
    `P-value` = map_dbl(shapiro_test, ~ .x$p)
  ) %>%
  select(
    Time,
    Margarine,
...
kable(
  normality_results,
  caption = "Shapiro-Wilk Normality Test for Cholesterol by Time and Margarine",
  digits = 3,
  booktabs = TRUE
) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 10)
